...
//        ig.music.add( this.grindSound, 'grindSound' );
    },
    
    update: function() {
        this.parent();
        if (!this.conveyor) {
            this.conveyor = ig.game.getEntitiesByType(EntityConveyor)[0];
        }
        if( this.currentAnim === this.anims.process && this.conveyor.onConveyorArr.length === 0 ) {
            console.log('idle!');
            this.currentAnim = this.anims.idle;
    ...
    drawEnergyBar: function() {
        ...
        ig.game.ctx.textAlign = 'center';
        ig.game.ctx.fillText(Math.round(ig.game.player.energy), x, y);
        ig.game.ctx.restore();    
    }
